...
def train(args, dataset, generator, discriminator):
    step = int(math.log2(args.init_size)) - 2 #-> 1
    resolution = 4 * 2 ** step
    loader = sample_data(
...
if __name__ == '__main__':
    ...
    #### Networks
    n_layers = int(math.log(args.max_size, 2)) - 1
        
    generator = nn.DataParallel(StyledGenerator(code_size, n_layers)).cuda()
    discriminator = nn.DataParallel(
        Discriminator(layers=n_layers, from_rgb_activate=not args.no_from_rgb_activate)
    ).cuda()

    g_running = StyledGenerator(code_size, n_layers).cuda()

...
def prepare(transaction, dataset, n_worker, sizes=(8, 16, 32, 64, 128, 256, 512, 1024)):
    resize_fn = partial(resize_worker, sizes=sizes) #-> partially initialized the function

    files = sorted(dataset.imgs, key=lambda x: x[0])
    files = [(i, file) for i, (file, label) in enumerate(files)] #-> ignore the labels
    total = 0
...
if __name__ == '__main__':
    ...
    with lmdb.open(args.out, map_size=1024 ** 3, readahead=False) as env:
        with env.begin(write=True) as txn:
            prepare(txn, imgset, args.n_worker, sizes=(8, 16, 32, 64, 128))
